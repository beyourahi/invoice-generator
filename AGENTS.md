...
- **Server-side scope** — `hooks.server.ts` handles auth sessions and security headers; `+page.server.ts` / `+layout.server.ts` load data; `src/routes/api/` holds the REST API for data persistence and the AI Copilot (model inference, quota, undo). The PDF pipeline (`builder.ts`, `generator.ts`, `sequential-download.ts`, `zip.ts`) stays client-side — never move PDF generation server-side.

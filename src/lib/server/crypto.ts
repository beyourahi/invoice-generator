/**
 * AES-GCM helpers for at-rest encryption of the user's Cloudflare Workers AI API token.
 *
...
export const deriveTokenKey = async (base64Key: string): Promise<CryptoKey> => {
...
export const encryptToken = async (plaintext: string, key: CryptoKey): Promise<Uint8Array> => {
...
export const decryptToken = async (blob: Uint8Array, key: CryptoKey): Promise<string> => {
	if (blob.length <= IV_LENGTH_BYTES) {
		throw new Error(
			`decryptToken: blob too short (${blob.length} bytes); expected > ${IV_LENGTH_BYTES}`
		);
	}
...
/** Display mask: `<first 10 chars>...`. The raw token never leaves the server unmasked. */
export const maskToken = (plaintext: string): string => {

...
<div class="fixed top-4 right-4 z-50 flex items-center gap-2 sm:top-6 sm:right-6">
	...
	<div class="relative">
		...
		>
			{#if isLoggingOut}
				<div
					class="h-4 w-4 animate-spin rounded-full border-2 border-zinc-500 border-t-transparent"
				></div>
			{:else}
				<svg
					class="h-4 w-4 text-red-400 transition-colors group-hover:text-white sm:h-[1.125rem] sm:w-[1.125rem]"
					viewBox="0 0 24 24"
					fill="none"
					stroke="currentColor"
					stroke-width="2"
